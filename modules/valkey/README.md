...
| gcs\_source\_uri | Backups that stored in Cloud Storage buckets. The Cloud Storage buckets need to be the same region as the instances | `list(string)` | `null` | no |
...
| managed\_backup\_source\_backup | Backups that generated and managed by memorystore | `string` | `null` | no |
